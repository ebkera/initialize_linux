...
# Below is the installation from Signals Repos (commented to not install on a popos machine), pop os recently removed sopport so we will need to use flatpack (install via popshop)
...
echo "  -README" >> ~/installation.log
wget -O ~/Templates/README.md https://raw.githubusercontent.com/ebkera/initialize_linux/main/Template_README.md
...
sudo apt-get install -y libscalapack-openmpi1
